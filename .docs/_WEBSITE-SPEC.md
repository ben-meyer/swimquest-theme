...
- Tapping the burger opens a panel that starts immediately _below_ the bar (so logo + toggle stay above it) and animates in via clip-path + opacity transition; panel content fades and slides up. Body scroll is locked while open via `no-scroll` on `<html>`.
